 ...
 */
...
#ifdef C_DEBUG
			CallFunction::AddCall(SegValue(cs), reg_eip, newcs, newip, GETIP);
#endif // #ifdef C_DEBUG
	...
	CASE_W(0xc2)												/* RETN Iw */
		reg_eip=Pop_16();
		CallFunction::NotifyReturn(SegValue(cs), reg_eip);
		reg_esp+=Fetchw();
		continue;
	CASE_W(0xc3)												/* RETN */
		reg_eip=Pop_16();
...
#ifdef C_DEBUG
			CallFunction::AddCall(SegValue(cs), fromOffset, SegValue(cs), (Bit16u)(reg_eip + addip), reg_eip);
#endif
...
#ifdef C_DEBUG
				CallFunction::AddCall(SegValue(cs), fromOffset, SegValue(cs), reg_eip, GETIP);
#endif

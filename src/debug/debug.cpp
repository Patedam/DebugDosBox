...
active(false),once(false),canActivate(true),
segment(0),offset(0),intNr(0),ahValue(0),alValue(0),
type(BKPNT_UNKNOWN) { };


void CBreakpoint::SetAddress(Bit16u seg, Bit32u off) 
{
...
const std::list<CBreakpoint*>& CBreakpoint::GetBreakPointList()
{
	return BPoints;
}

void CBreakpoint::SerializeAll(const char* filename, bool isReading)
{
	std::string fileNameStr(filename);
	fileNameStr.append(".dbx");
	
	if (isReading == false)
	{
		SDL_RWops *file = SDL_RWFromFile(fileNameStr.c_str(), "wb");
		size_t size = BPoints.size();
		SDL_RWwrite(file, &size, sizeof(size_t), 1);
		for (CBreakpoint* bp : BPoints)
		{
			SDL_RWwrite(file, bp, sizeof(CBreakpoint), 1);
		}
		SDL_RWclose(file);
	}
	else
	{
		SDL_RWops *file = SDL_RWFromFile(fileNameStr.c_str(), "rb");
		if (file)
		{
			DeleteAll();

			size_t size = 0;
			SDL_RWread(file, &size, sizeof(size_t), 1);
			for (Bitu i = 0; i < size; ++i)
			{
				CBreakpoint* bp = new CBreakpoint();
				SDL_RWread(file, bp, sizeof(CBreakpoint), 1);
				BPoints.push_front(bp);
			}
			SDL_RWclose(file);
		}
...
// Statics
std::deque<CallFunction*> CallFunction::CallStack;

void CallFunction::AddCall(Bitu fromSeg, Bitu fromOff, Bitu toSeg, Bitu toOff, Bitu retOff)
{
	CallFunction* cf = new CallFunction();
	// TODO : Compute address because multiple seg and offset can point to same address
	cf->FromSegment = fromSeg;
	cf->ToSegment = toSeg;
	cf->FromOffset = fromOff;
	cf->ToOffset = toOff;
	cf->ReturnOffset = retOff;
...
void CallFunction::NotifyReturn(Bitu seg, Bitu off)
{
	if (CallStack.empty() == false)
	{
		CallFunction* cfBack = CallStack.back();
		// Todo use phys address and not seg et off
		if (cfBack->GetFromSegment() == seg && cfBack->GetReturnOffset() == off)
		{
			CallStack.pop_back();
			delete cfBack;
		}

		// TODO: Check if we find it somewhere and assert if so because it should always be back
	}
}

void CallFunction::ClearCallStack()
{
	for (CallFunction* cfToDelete : CallStack)
	{
		delete cfToDelete;
	}
	CallStack.clear();
...
const std::deque<CallFunction*>& CallFunction::GetCallStack()
{
	return CallStack;
}
...
static bool StepOver()
{
	...
	OpInfo opInfo;Bitu size;
	size=DasmI386(&opInfo, start, reg_eip, cpu.code.big);

	if (strstr(opInfo.dline,"call") || strstr(opInfo.dline,"int") || strstr(opInfo.dline,"loop") || strstr(opInfo.dline,"rep")) {
...
static void DrawCode(void) {
	...
	OpInfo opInfo;Bitu size;Bitu c;
	...
	for (int i=0;i<10;i++) {
		...
		char empty_res[] = { 0 };
		char* res = empty_res;
		if (showExtend) res = AnalyzeInstruction(opInfo.dline, saveSel);
		// Spacepad it up to 28 characters
		size_t dline_len = strlen(opInfo.dline);
		if(dline_len < 28) for (c = dline_len; c < 28;c++) opInfo.dline[c] = ' '; opInfo.dline[28] = 0;
		waddstr(dbg.win_code, opInfo.dline);
...
static Bits CurrentRet = 0;
static bool CallDebug_CheckKeys = true;
void DEBUG_Run()
{
	debugging = false;
...
void DEBUG_StepOut()
{
	if (CallFunction::GetCallStack().size() > 0)
	{
		CallFunction* cf = CallFunction::GetCallStack().back();
		CBreakpoint::AddBreakpoint(cf->GetFromSegment(), cf->GetReturnOffset(), true);
		DEBUG_Run();
	}
}
				...
				if (codeViewData.cursorPos>0) codeViewData.cursorPos--;
					...
					if(codeViewData.useEIP) {
						for (; bytes < 10; bytes++) {
							...
							size = DasmI386(&opInfo, start, newEIP, cpu.code.big);
...
Bitu DEBUG_Loop(void) {
	ProcessEvents();
	Debug_UpdateAndDraw(); // Draw at 30fps to make everything smooth and to draw when dosbox is not drawing anything
	...
	if (CallDebug_CheckKeys)
	{
		CurrentRet = DEBUG_CheckKeys();
	}
	else
	{
		CallDebug_CheckKeys = true;
		if (CurrentRet < 0) return CurrentRet;
		if (CurrentRet > 0) {
			if (GCC_UNLIKELY(CurrentRet >= CB_MAX))
				CurrentRet = 0;
			else
				CurrentRet = (*CallBack_Handlers[CurrentRet])();
			if (CurrentRet) {
				exitLoop = true;
				CPU_Cycles = CPU_CycleLeft = 0;
				return CurrentRet;
...
void DEBUG_Enable(bool pressed) {
	...
	KEYBOARD_ClrBuffer();
	GFX_LosingFocus(); // Lose focus from game viewport
	DEBUG_OnDebugEnable();
}
...
static void LogInstruction(Bit16u segValue, Bit32u eipValue,  ofstream& out) {
	...
	if (showExtend && (cpuLogType > 0) ) {
		res = AnalyzeInstruction(opInfo.dline,false);
		if (!res || !(*res)) res = empty;
		Bitu reslen = strlen(res);
		if (reslen<22) for (Bitu i=0; i<22-reslen; i++) res[reslen+i] = ' '; res[22] = 0;
	};
	Bitu len = strlen(opInfo.dline);
	if (len<30) for (Bitu i=0; i<30-len; i++) opInfo.dline[len + i] = ' '; opInfo.dline[30] = 0;

	// Get register values

	if(cpuLogType == 0) {
		out << setw(4) << SegValue(cs) << ":" << setw(4) << reg_eip << "  " << opInfo.dline;
	} else if (cpuLogType == 1) {
		out << setw(4) << SegValue(cs) << ":" << setw(8) << reg_eip << "  " << opInfo.dline << "  " << res;
		...
		out << setw(4) << SegValue(cs) << ":" << setw(8) << reg_eip << "  " << opInfo.dline << "  " << res << "  " << ibytes;
	}
...
public:
	...
	void Run(void)
	{
		...
		CBreakpoint::SerializeAll(filename);
...
void DEBUG_HeavyLogInstruction(void) {
	...
	PhysPt start = GetAddress(SegValue(cs),reg_eip);
	OpInfo opInfo;
	DasmI386(&opInfo, start, reg_eip, cpu.code.big);
	char* res = empty;
	if (showExtend) {
		res = AnalyzeInstruction(opInfo.dline,false);
		if (!res || !(*res)) res = empty;
	...
	Bitu len = strlen(opInfo.dline);
	if (len < 30) for (Bitu i=0; i < 30-len; i++) opInfo.dline[len+i] = ' ';
	opInfo.dline[30] = 0;

	TLogInst & inst = logInst[logCount];
	strcpy(inst.dline, opInfo.dline);

...
SDL_Surface* SDL_SetVideoMode_Wrap(int width,int height,int bpp,Bit32u flags){
	...
	if((i_flags&SDL_WINDOW_OPENGL) && !(flags&SDL_WINDOW_OPENGL) && (i_flags&SDL_WINDOW_FULLSCREEN) && !(flags&SDL_WINDOW_FULLSCREEN)){
		GFX_SwitchFullScreen();
		return SDL_SetVideoMode_Wrap(width,height,bpp,flags);
	}

	//PXX
	if ((i_flags&SDL_WINDOW_OPENGL) && !(flags&SDL_WINDOW_OPENGL) && height==i_height && width==i_width && height==480) {
		height++;
	}
#endif //WIN32
#endif //SETMODE_SAVES
	SDL_Surface* s = SDL_CreateRGBSurface(flags, width, height, bpp, 0xff000000, 0x00ff0000, 0x0000ff00, 0x000000ff);//SDL_SetVideoMode(width,height,bpp,flags);
#if SETMODE_SAVES
...
void GFX_SwitchFullScreen(void) {
	...
	if (sdl.desktop.fullscreen) {
		if (!sdl.mouse.locked) Input_CaptureMouse();
#if defined (WIN32)
...
//	if(sdl.desktop.fullscreen) Input_CaptureMouse();
//
//	if (output == "surface") {
...
//	sdl.surface=SDL_SetVideoMode_Wrap(640,400,0, SDL_WINDOW_FULLSCREEN);
//	if (sdl.surface == NULL) {
...
void GFX_Events() { // Now named ProcessEvents

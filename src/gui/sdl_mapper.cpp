...
public:
	...
	void BindName(char * buf) {
		sprintf(buf, "Key %s", SDL_GetKeyName((Bitu)key));
	}
...
public:
	...
	CBind * CreateConfigBind(char *& buf) {
		...
		if (usescancodes) 
		{
			code = (Bitu)SDL_GetScancodeFromKey((SDL_Keycode)code);
		}
...
{ "period",SDLK_PERIOD },{ "slash",SDLK_SLASH },{ "printscreen",SDLK_PRINTSCREEN },
{ "scrolllock",SDLK_SCROLLLOCK },{ "pause",SDLK_PAUSE },{ "pagedown",SDLK_PAGEDOWN },
...
{ "kp_8",SDLK_KP_8 },{ "kp_9",SDLK_KP_9 },{ "numlock",SDLK_NUMLOCKCLEAR },
...
#define NUM_KEYCODE 240
static void CreateBindGroups(void) {
	bindgroups.clear();
	new CKeyBindGroup(NUM_KEYCODE);
	if (joytype != JOY_NONE) {
#if defined (REDUCE_JOYSTICK_POLLING)
...
void MAPPER_RunInternal() {
	...
	//SDL_SetPalette(mapper.surface, SDL_LOGPAL | SDL_PHYSPAL, map_pal, 0, 6); // Fixme ?
	if (last_clicked) {
...
#endif
	if (mousetoggle) Input_CaptureMouse();
